...
The harness runs 23 POCs on the primary test environment (kernel 6.12.54-linuxkit aarch64 via Docker Desktop). 18 produce `_PROVEN` proof markers on stdout there; 2 (ch06 LSM, ch12 LSM) skip on linuxkit at runtime because linuxkit does not load SELinux policy and is not built with `CONFIG_MODULE_SIG_FORCE`, and fire on the Fedora 42 aarch64 QEMU VM secondary (driven by `dBPF-pocs/run-qemu-tests.sh` + `dBPF-pocs/qemu-runner.sh`); 2 Act-4 additions (ch23 TPM unseal, ch25 IMDS via XDP) are PROVEN on the Ubuntu 6.17.0-29-generic aarch64 Lima VM (final verification environment) — ch23 via kprobe attachment with entry intercept events, ch25 via XDP mock IMDSv2 on `lo`; 1 Act-4 addition (ch24 bpf_token delegation) SKIP in all test environments because `CONFIG_BPF_TOKEN=n` on Ubuntu 6.17 and Fedora 6.17 kernels — the feature requires explicit opt-in at build time and no available kernel had it enabled. **Final tally: 24 PROVEN across all environments, 1 SKIP (ch24).** Chapter 21 accounts for every skip. This chapter walks the proven cases, puts them in five primitive classes, and classifies each into one of four honesty categories. The five-class taxonomy and the four-category system together are the durable artifact of this book.

Act 4 extended the scope of the catalog along two axes without changing the taxonomy. The on-host, software-only primitives of Chapters 1–18 all manipulate state that lives inside the running Linux kernel's address space — credentials, policy decisions, syscall returns, user buffers, packet paths. Act 4's three chapters test two new axes: (a) hardware-rooted key material that transits the kernel during TPM-backed trusted-key operations (ch23), and (b) off-host boundaries that an on-host `CAP_BPF` holder can reach through — the cloud metadata service for workload identity (ch25), and the delegated-capability boundary between a privileged process and an unprivileged client that holds its token (ch24). All three fit into the existing five-class taxonomy: ch23 is Class III (ringbuf exfil, now of hardware-rooted bytes), ch25 is Class IV (packet-path interception, now at a cross-boundary address), and ch24 is a threat-model subversion using Class III-shaped mechanics against a delegated-capability surface that did not exist when the taxonomy was drafted. The taxonomy accommodates Act 4 by row-addition; no class needed to be added.
...
- **REAL** (16 POCs; default category): Hooks the actual kernel subsystem the chapter targets, and can observe or mutate. These are the strongest demonstrations — the hook, the surface, and the effect are all production-representative. Members: ch01 (cap_capable + `bpf_send_signal`), ch02 (overlayfs copy-up), ch04 (phantom syscall via `tp/syscalls/sys_enter_write`), ch05 (cgroup `cpu.stat` rewrite), ch05b (XDP ghost NIC), ch07 (devcgroup kprobe on `devcgroup_check_permission`), ch08 (keyring heist via kprobe), ch08k (keyring heist kprobe variant), ch09 (PID-NS cross-mapping + `bpf_send_signal`), ch10 (getdents64 inode cloak), ch11 (IRQ timing channel), ch12 (signed-driver-swap LSM — skips on linuxkit, fires in Fedora QEMU), ch15 (netns VLAN ghost XDP), plus the three Act 4 chapters ch23 (TPM trusted-key plaintext capture — skips in both test environments for lack of TPM hardware), ch24 (bpf_token delegation — skips in both test environments on Fedora 42's cloud-init `BPF_TOKEN_CREATE`=`EOPNOTSUPP`), and ch25 (IMDS credential capture via XDP — fires on the Fedora QEMU VM in mock IMDSv2 mode with `CH25_PROVEN access_key_captured=yes`).
- **OBSERVER** (4 POCs): Hooks the real subsystem but cannot mutate — the error-injection allowlist blocks `bpf_override_return` on the target, or atomic context prevents mutation. The POC reads kernel-internal state and exfiltrates it, which is Class III behavior. Members: ch03 (audit record exfil), ch06 (SELinux LSM — skips on linuxkit, fires in Fedora QEMU), ch06o (SELinux kprobe observer), ch16 (seccomp decision leak).
- **ILLUSION** (3 POCs): Hooks the real syscall entry point but only forges the return value — kernel state is unchanged. The illusion fools userspace consumers of the syscall return but does not alter what the kernel actually did. Members: ch12s (`__arm64_sys_finit_module` return forge), ch14 (`__arm64_sys_sched_setscheduler` return forge), ch18 (`__arm64_sys_getuid`/`__arm64_sys_geteuid` return forge). Note: all three ILLUSION POCs hook aarch64-specific syscall symbols (`__arm64_sys_*`); on x86 the equivalent symbols are `__x64_sys_*`.
- **ANALOG** (0 POCs): No analog variants are registered in the current harness. Earlier drafts included analog PoCs for subsystems missing from aarch64 linuxkit (RAPL, ACPI) and for LSM shapes on kernels without SELinux enforcement; these were removed in the cleanup that also produced the current 23-PoC manifest. The category is retained in the dataclass schema for forward compatibility.

The category is orthogonal to the primitive class. A Class I primitive can be `real` (ch01, ch12) or `illusion` (ch14, ch18, ch12s). A Class III primitive can be `real` (ch04, ch08k) or `observer` (ch03, ch06o). The category tells you how much to trust the demonstration; the class tells you what the primitive does.
...
Representative chapters: ch01 (cap_capable override — marker `CH01_WEAPON_PROVEN flips=N`, category `real`), ch14 (`__arm64_sys_sched_setscheduler` forge — marker `SCHED_WEAPON_PROVEN flips=N`, category `illusion`, aarch64-only), ch18 (`__arm64_sys_getuid`/`__arm64_sys_geteuid` forge — marker `TOKEN_FORGE_PROVEN uid_forges=N`, category `illusion`, aarch64-only), ch12 (signed-driver-swap LSM on `kernel_read_file` — fires on Fedora 42 aarch64 QEMU VM where `CONFIG_MODULE_SIG_FORCE=y`; skips on linuxkit), ch12s (`__arm64_sys_finit_module` return forge — marker `CH12_CONCEPT_PROVEN`, category `illusion`, aarch64-only), and ch06 (LSM `fmod_ret` on SELinux hooks — fires on the Fedora QEMU VM with SELinux enforcing; skips on linuxkit which does not load SELinux policy).
...
Representative chapters: ch03 (FUSE/audit record exfil via fentry — marker `CH03_PROVEN variant=fentry before=N after=M`, category `observer`), ch04 (phantom syscall field leakage via `__arm64_sys_write` — marker `CH04_PROVEN leaked_fields=N`, category `real`), ch06o (kprobe observer of SELinux hooks — marker `CH06_PROVEN hook=...`, category `observer`), ch08 and ch08k (keyring description leakage via kprobe on `key_task_permission` — markers `CH08_PROVEN` and `CH08_CONCEPT_PROVEN events=N`, both category `real`), ch09 (PID-namespace cross-mapping via `SEC("raw_tp/sched_process_fork")` — marker `CH09_PROVEN host_pid=N mapped=yes`, category `real`), ch11 (per-IRQ timing sidechannel via kprobes on `handle_irq_event*` — marker `CH11_PROVEN events=N unique=M per_event_timing=yes`, category `real`), ch16 (seccomp decision for another TID via kprobe on `__secure_computing` — marker `SECCOMP_SIDECHANNEL_PROVEN events=N`, category `observer`).
...
Five POCs implement Class I return-override across the two environments. ch12 (LSM fmod_ret) fires only on Fedora 42 QEMU; the other four (ch01, ch14, ch18, ch12s) plus ch06 (Fedora-only) round out the class. Each uses either a kretprobe with `bpf_override_return` on a function in the error-injection allowlist, or BPF LSM `fmod_ret` on an LSM hook. The detailed walkthroughs below cover the four that are discussed in depth.

**ch01 — cap_capable.** The POC in `dBPF-pocs/pocs/ch01-mirror-controls/` was the first chapter in the book, and it hit the first honest limit. `cap_capable` is not in `/sys/kernel/debug/error_injection/list` on linuxkit 6.12 aarch64 — the kprobe attaches, but `bpf_override_return` from the kretprobe is silently ignored by the kernel. Rather than publish an observer that only logs, the POC was upgraded to `bpf_send_signal(SIGUSR1)` from the kretprobe when the observed deny lands on a targeted tgid. The marker is `CH01_WEAPON_PROVEN flips=N signals=N` and the two fields count signal deliveries, not return-value flips — so calling this Class I is slightly generous, but the primitive is shaped like Class I (observe the deny at the API boundary, act on it). The unregistered variant in `ch01-mirror-controls-lsm/` attaches `SEC("lsm/inode_permission")` on 6.14+ kernels and *does* implement a real fmod_ret override, but that variant is not in the harness. Category: `real`.
...
Across these five, the common pattern: userspace reads a forged API return. Kernel-side enforcement reads real state. Anything that gates decisions on the kernel's actual state (credentials, policy, loaded modules) is unaffected. Anything that trusts the libc syscall wrapper's return value is fooled. Three of the five (ch12s, ch14, ch18) are categorized as `illusion` because they hook real aarch64-specific syscall entry points but only forge the return value — the kernel never actually performed the requested operation. Two (ch01 and ch12) are `real`; ch12 additionally requires the Fedora 42 QEMU environment because linuxkit does not enforce module signatures.

## Detailed walkthrough: Class II — userspace buffer rewrite

Two POCs, one primitive. `bpf_probe_write_user` waits for a syscall to finish, looks up the userspace buffer the kernel just wrote to, and mutates it before the caller regains control. Both (ch05, ch10) are `real` category.
...
Every primitive in this book is one of three motions: change the syscall return, rewrite the user buffer, or copy the decision out-of-band. These three motions are what `CAP_BPF` grants. The twenty-five POCs attempted across the eighteen on-host chapters plus Act 4 cross-boundary chapters (18 fire on linuxkit; 2 fire on the Fedora 42 QEMU VM; 2 — ch23 and ch25 — are PROVEN on the Ubuntu 6.17 aarch64 Lima VM; 1 — ch24 — SKIP in all environments due to `CONFIG_BPF_TOKEN=n`) are demonstrations that granting `CAP_BPF` grants access to all three motions across a representative surface of kernel subsystems — and, as Act 4 extends, across hardware-rooted key material and off-host cloud-metadata boundaries. That is the capability, operating as designed. The four-category system (`real`, `observer`, `illusion`, `analog`) ensures honest labelling of what each demonstration actually proved. The blue-team implication is in chapter 22.

## Master table

All 23 registered POCs, mapped to category, primitive class, SEC hooks, BPF primitive used, and effect. Status is `effect_demonstrated` on linuxkit unless otherwise noted.
...
| ch23 | real | III | kprobe `tpm2_unseal_trusted` | ringbuf exfiltration of unsealed key bytes | **PROVEN** on Ubuntu 6.17.0-29-generic aarch64 (Lima VM): kprobe attaches, entry intercept events observed (`CH23_PROVEN hook=attached kind=kprobe-on-tpm2_unseal_trusted sym-confirmed`). Skips on linuxkit (no TPM symbol). Lima VM vTPM proxy not boot-registered, so keyctl trusted path unavailable; full byte-capture proof requires a host with a registered TPM backend. | Act 4; Class III extended to hardware-rooted key material |
| ch24 | real | — | tracepoint `tp:syscalls/sys_enter_getuid` attached via delegated `BPF_TOKEN_CREATE` fd | SCM_RIGHTS handoff of a bpf-token to an unprivileged client | **SKIP — `CONFIG_BPF_TOKEN=n` on all available kernels** (`CH24_SKIP`). Ubuntu 6.17 and Fedora 6.17 kernels do not have this option enabled; `BPF_TOKEN_CREATE` returns `ENOSYS` (command absent). Requires kernel 6.9+ built with `CONFIG_BPF_TOKEN=y`. C code is production-reviewed; the skip is a build-configuration gap, not a code issue. | Act 4; threat-model subversion via delegated capability; does not fit cleanly into Classes I–V |
| ch25 | real | IV | `SEC("xdp")` on `lo` | ringbuf exfil of IMDSv2 access keys and tokens | **PROVEN** on Ubuntu 6.17.0-29-generic aarch64 (Lima VM) in mock IMDSv2 mode (`CH25_PROVEN access_key_captures=1 token_captures=1 role=demo-role`). Skips on linuxkit. | Act 4; Class IV extended to cross-boundary cloud metadata |

**Category counts** (from literal `category=` assignments in `proof.py` plus the `"real"` default): real=16, observer=4, illusion=3, analog=0 (total 23). **Status counts**: 18 demonstrate on linuxkit; 2 (ch06 LSM, ch12 LSM) skip on linuxkit and demonstrate on the Fedora 42 aarch64 QEMU VM; 2 (ch23, ch25) skip on linuxkit and are PROVEN on the Ubuntu 6.17 aarch64 Lima VM; 1 (ch24) SKIP in all environments (`CONFIG_BPF_TOKEN=n`); **across-environment total 24 PROVEN, 1 SKIP (ch24), 0 failures**.
