...
| ch23 | real | III | kprobe `tpm2_unseal_trusted` | ringbuf exfiltration of unsealed key bytes | **Skips on linuxkit and on the Fedora 42 QEMU VM** (`CH23_SKIP reason="no /dev/tpm0 or /dev/tpmrm0"`) — fires on any host with a TPM device or swtpm passthrough | Act 4; Class III extended to hardware-rooted key material |
| ch24 | real | — | tracepoint `tp:syscalls/sys_enter_getuid` attached via delegated `BPF_TOKEN_CREATE` fd | SCM_RIGHTS handoff of a bpf-token to an unprivileged client | **Skips on linuxkit and on the Fedora 42 QEMU VM** (`CH24_SKIP reason="BPF_TOKEN_CREATE failed: Operation not supported"`) under cloud-init despite `/proc/self/ns/user` matching pid 1 — production-reviewed C code, environmental gap documented in Chapter 24 | Act 4; threat-model subversion via delegated capability; does not fit cleanly into Classes I–V |
| ch25 | real | IV | `SEC("xdp")` on `lo` | ringbuf exfil of IMDSv2 access keys and tokens | Skips on linuxkit; **fires on the Fedora 42 aarch64 QEMU VM** in mock IMDSv2 mode (`CH25_PROVEN access_key_captured=yes token_captured=yes role=demo-role`) | Act 4; Class IV extended to cross-boundary cloud metadata |

**Category counts** (from literal `category=` assignments in `proof.py` plus the `"real"` default): real=16, observer=4, illusion=3, analog=0 (total 23). **Status counts**: 18 demonstrate on linuxkit; 2 (ch06 LSM, ch12 LSM) skip on linuxkit and demonstrate on the Fedora 42 aarch64 QEMU VM; 1 (ch25) skips on linuxkit and demonstrates in Fedora QEMU in mock mode; 2 (ch23, ch24) skip in both environments with named Act-4 reasons; across-environment total 19 demonstrated, 4 skipped with honest reasons, 0 failures.
