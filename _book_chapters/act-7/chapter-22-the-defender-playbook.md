...
Cross-references: this step directly mitigates every Class I primitive — ch01, ch06 (LSM, Fedora QEMU), ch07, ch12 (LSM, Fedora QEMU), ch12s (syscall illusion), ch14, ch18. ch10 is Class II (d_reclen swallow returns modified data, not a return value), covered in the row below. ch08k and ch08 at `dBPF-pocs/pocs/ch08-keyring-heist-kprobe/` and `dBPF-pocs/pocs/ch08-keyring-heist/` are Class III (exfil), not Class I, and are unaffected by this step; restrict them via step 3's LSM gate.
...
- **`mknod` return** — ch07 (`dBPF-pocs/pocs/ch07-devcgroup-houdini/`). A container-runtime convenience script reading `mknod` success and assuming the device node now exists — similar shape.
...
|-------|----------|-------------------|------------------|--------------------------|
| **I — Return-value override** | ch01, ch06 (LSM; fires only on Fedora 42 aarch64 QEMU with SELinux enforcing), ch07, ch12 (LSM; fires only on Fedora 42 aarch64 QEMU with module signing), ch12s (syscall illusion), ch14, ch18 | BPF LSM gate on `fmod_ret`, `kretprobe`, and `bpf_override_return` (step 3, 6); strip `ALLOW_ERROR_INJECTION` annotations at kernel build (step 6) | Audit `bpf(2)` with `a0=5` (`BPF_PROG_LOAD`) and ship off-host (step 5); baseline BPF program set and alert on new kretprobes (step 4) | Audit log shows `BPF_PROG_LOAD` by unexpected UID; baseline diff shows new kretprobe on syscall wrapper; post-check ground truth in orchestrator (step 7) |
...
| **III — Ringbuf exfiltration** | ch03, ch04, ch06o, ch08, ch08k, ch09, ch11, ch16, **ch23 (TPM unseal heist — PROVEN on Ubuntu 6.17 aarch64 Lima VM: kprobe attaches to `tpm2_unseal_trusted`, entry intercept events observed; full byte-capture on hosts with a boot-registered TPM backend)** | Cannot prevent at kernel level once the peer has `CAP_BPF`; minimize holders (step 2); ship off-host synchronously (step 5); enclaves for true secrets (step 8); for ch23 specifically, treat any `CAP_BPF` holder on a host with TPM-backed trusted keys as having seen those keys in plaintext — rotate the key set on `CAP_BPF` holder install/update | Audit `bpf(2)` for `BPF_MAP_CREATE` of type `BPF_MAP_TYPE_RINGBUF` and baseline BPF program set; alert on any kprobe attachment to `tpm2_unseal_trusted` or `trusted_key_unseal` outside of known TPM test tooling | Record ringbuf maps created and by whom; correlate with credential access events in other subsystems; for ch23, correlate TPM audit log entries with BPF load events |
| **IV — Packet-path interception (XDP)** | ch05b, ch15, **ch25 (IMDS credential capture via XDP — fires on the Fedora 42 aarch64 QEMU VM with mock IMDSv2 on `lo`; any host where a `CAP_BPF` holder can attach XDP to the interface carrying metadata-service traffic is exposed)** | BPF LSM gate on `BPF_PROG_LOAD` for type `BPF_PROG_TYPE_XDP`; netlink audit on netdev attach; for ch25, treat the host's cloud-metadata interface (typically the default route's egress NIC) as a high-value attach target and alert on any XDP program attached to it | Audit `bpf(2)`; baseline; alert on new XDP programs via `bpftool net show -j`; watch for XDP attach to `lo` or to the primary egress NIC | `bpftool net show` output diffs vs. baseline; `tcpdump` loss vs. `cat /proc/net/dev` counters (packets seen by netdev but not by tcpdump indicates XDP drop); for ch25, instance credential issuance rotation metrics correlated with unexpected use sites |
...
| **Threat-model subversion via delegated capability** (new in Act 4; does not fit cleanly into Classes I–V because the mechanism is a legitimate kernel feature — `BPF_TOKEN_CREATE` — used for its documented purpose, with the subversion being the trust model around who receives the token fd) | ch24 (bpf_token delegation — **SKIP in all test environments: `CONFIG_BPF_TOKEN=n` on Ubuntu 6.17 and Fedora 6.17 kernels**; the feature requires an explicit build-time opt-in not present in any available kernel; the C code is production-reviewed and the primitive is real on any kernel built with `CONFIG_BPF_TOKEN=y`) | Audit every `BPF_TOKEN_CREATE` call and treat the token fd as a transferable capability — track it across SCM_RIGHTS handoffs; restrict which processes may call `BPF_TOKEN_CREATE` via BPF LSM `bpf_token_create` hook if available; do not grant `CAP_BPF` to services that accept unauthenticated client connections | Audit `bpf(2)` for cmd `BPF_TOKEN_CREATE` (cmd 36); audit SCM_RIGHTS on unix sockets owned by `CAP_BPF`-holding processes; correlate token creation with unexpected client uids using the resulting fd | Kernel audit log shows token create by a server process followed by token use by a client with a different uid; orchestrator's capability inventory (step 2) shows a delegated-capability footprint that was not present at baseline |
