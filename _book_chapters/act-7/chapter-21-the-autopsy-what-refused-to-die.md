...
Four primitives do not produce effects on the linuxkit 6.12 aarch64 primary environment. Not because the BPF code is wrong; because the linuxkit kernel environment does not host the surface the primitive needs to grip. ch06 (BPF LSM fmod_ret on SELinux hooks) needs a running SELinux policy that linuxkit does not load; ch12 (BPF LSM fmod_ret on `kernel_read_file`) needs module-signature enforcement that linuxkit is not built with. Both fire on the Fedora 42 aarch64 QEMU VM secondary. Act 4 added three more cross-boundary PoCs: ch23 (TPM unseal heist) and ch25 (IMDS XDP harvest) are both PROVEN on the Ubuntu 6.17.0-29-generic aarch64 Lima VM — ch23 via kprobe attachment with entry intercept events (vTPM proxy not boot-registered, so keyctl path unavailable but the primitive itself is proved), ch25 via XDP mock IMDSv2 on `lo`. ch24 (bpf_token delegation) SKIP in all test environments because `CONFIG_BPF_TOKEN=n` on Ubuntu 6.17 and Fedora 6.17 kernels — this is a build-configuration gap, not a runtime rejection. The goal of this chapter is to make every skip reproducible and environment-grounded.
...
**Shape C — target subsystem absent from this kernel build.** The entire subsystem the primitive hooks is compiled out. No symbols, no sysfs interface, no call sites. Earlier drafts of this book registered aarch64-incompatible PoCs (powercap/RAPL, ACPI) that fell under this shape; those were retired from the harness in the cleanup that produced the current manifest. Note: ch23 (TPM unseal heist) was previously described as Shape C on both test environments. Final verification on Ubuntu 6.17 aarch64 (Lima VM) shows `tpm2_unseal_trusted` present in kallsyms and the kprobe attaching and firing entry intercept events — ch23 is now PROVEN (Shape C does not apply on this substrate). Shape C no longer applies to any registered PoC; it is retained as a category for documentation completeness.

**Shape D — syscall-level rejection despite passing all documented preconditions (Fedora 42 cloud-init).** The BPF program compiles and builds; all documented prerequisites (capability set, namespace placement, kernel version, build options) are satisfied; and still the kernel's `bpf(2)` syscall returns an error code the documented path does not predict. This is the shape Chapter 24's bpf_token delegation PoC hits on Fedora 42 aarch64 kernel 6.14 under cloud-init: `BPF_TOKEN_CREATE` returns `EOPNOTSUPP`, seven iterations of increasingly specific failure-mode chasing later, despite the server process holding `CAP_BPF`+`CAP_SYS_ADMIN` and `/proc/self/ns/user` matching pid 1's init user namespace. The C code is production-reviewed; the primitive is real on a kernel where the path is live. The Fedora-on-cloud-init observation is an environmental gap, not a mainline-kernel bug — specifically a gap in how cloud-init's early-boot userns management interacts with 6.14's bpf-token delegation path. Shape D is distinct from Shape A (enforcement point inactive at runtime), B (BTF or symbol missing), and C (subsystem absent) because the subsystem, the symbol, the BTF, and the policy are all present — the rejection is at the syscall layer itself.

**Shape E — feature not compiled in (Ubuntu/Fedora 6.17).** The primitive depends on a specific kernel sub-feature that requires an explicit `CONFIG_*=y` build option that is absent in all available kernels. ch24 hits this on Ubuntu 6.17 and Fedora 6.17: `CONFIG_BPF_TOKEN` is not set, so `BPF_TOKEN_CREATE` does not exist in the `bpf()` syscall dispatch table and returns `ENOSYS`. The feature landed in kernel 6.9 but is an opt-in build option; no mainstream distro kernel has yet enabled it by default. Shape E is distinct from Shape D (feature compiled in, runtime rejection) and Shape C (whole subsystem absent).

Shape A is orthogonal to Shapes B, C, D, and E. On linuxkit versus Fedora QEMU, the axis that matters for the Act 1–3 skips is runtime enforcement-point activity (Shape A). For ch24 on Ubuntu/Fedora 6.17, the axis is Shape E (CONFIG_BPF_TOKEN not compiled in).
...
Four skips across the full verification run, three active shapes:
...
- **Shape D (syscall-level rejection, Fedora-on-cloud-init only):** Not the final blocker for ch24 on 6.17. See Shape E.
- **Shape E (feature not compiled in):** ch24. `CONFIG_BPF_TOKEN=n` on Ubuntu 6.17 and Fedora 6.17 kernels — `BPF_TOKEN_CREATE` returns `ENOSYS`. The feature requires explicit opt-in at build time. Any kernel built with `CONFIG_BPF_TOKEN=y` will see ch24 fire end-to-end.

Not a skip (previously described as Shape C): ch23. Final verification on Ubuntu 6.17 aarch64 (Lima VM) shows the kprobe attaches and entry intercept events fire — **ch23 is PROVEN**. The Lima VM's vTPM proxy was not boot-registered, so the `keyctl add trusted` path is unavailable, but the BPF primitive itself is confirmed. ch25 is also PROVEN on the Lima VM via XDP mock IMDSv2 on `lo`.

The honest final ledger: 18 primitives fire on linuxkit aarch64; 2 (ch06, ch12) fire on the Fedora 42 QEMU VM; 2 (ch23, ch25) are PROVEN on the Ubuntu 6.17 aarch64 Lima VM; 1 (ch24) SKIP in all environments (Shape E, `CONFIG_BPF_TOKEN=n`). **Cross-environment total: 24 PROVEN, 1 SKIP.** The distinction between "works" and "does not work" is not a property of the primitive alone — it is a relation between the primitive and a specific kernel build.
