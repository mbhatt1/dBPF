...
> **Proof status**: SKIP; `CONFIG_BPF_TOKEN=n` on every available kernel. BPF token delegation (`BPF_TOKEN_CREATE`, command 36) requires kernel 6.9+ compiled with `CONFIG_BPF_TOKEN=y`. None of the tested kernels — linuxkit 6.12, Fedora 42 (6.14), or Ubuntu 6.17.0-29-generic aarch64 — had this option enabled. The syscall returns `ENOSYS`; the command does not exist in the `bpf()` dispatch table. The C code has been reviewed and reads correctly. The skip is a build-configuration gap, not a code issue, and a kernel built with `CONFIG_BPF_TOKEN=y` should exercise the primitive — but that has not been demonstrated here, so this chapter is a genuine skip rather than a proven fire. Marker: `CH24_SKIP reason="CONFIG_BPF_TOKEN=n on all available kernels"`.
...
That last point is exactly what applied to every kernel actually available. On linuxkit 6.12, Fedora 42 (6.14), and Ubuntu 6.17, `CONFIG_BPF_TOKEN` is not compiled in at all. `BPF_TOKEN_CREATE` returns `ENOSYS`; the command simply does not exist in the dispatch table. That is why the harness marks this chapter as a skip rather than a fire.

The lesson underneath all of this: **the primitive's reachability is governed by more than kernel version and capability set**. The version check is necessary and the capability check is necessary, but neither is sufficient. The actual `cred->user_ns` has to satisfy the kernel's check too, and that can diverge from what userspace tooling reports. A defender asking "is my host at risk?" has to check not just "does my kernel have `CONFIG_BPF_TOKEN=y`" but "does `BPF_TOKEN_CREATE` actually succeed from my workload's real runtime context."
