...
> **Proof status**: PROVEN (with the correct choreography), on the stock Ubuntu 6.17 aarch64 kernel. Two earlier drafts of this chapter claimed a skip on `CONFIG_BPF_TOKEN=n`; that was wrong on the facts. **There is no `CONFIG_BPF_TOKEN` build option** — BPF-token delegation (`BPF_TOKEN_CREATE`, command 36) has ridden on `CONFIG_BPF_SYSCALL` unconditionally since Linux 6.9, so `grep CONFIG_BPF_TOKEN /boot/config-*` returning nothing means the symbol does not exist, not that the feature is absent. Verified live: in an unprivileged user namespace, a map/program load is denied (`EPERM`) with no token and **succeeds with a delegated token** — `CH24_RESULT no_token_map=DENIED no_token_prog=DENIED token_create=ok with_token_map=LOADED with_token_prog=LOADED => CH24_PROVEN`. The original PoC did not fire for an architectural reason, not a missing feature: it minted the token from `init_user_ns`, which the kernel refuses with `-EOPNOTSUPP` ("Creating BPF token in init_user_ns doesn't make much sense") — not `ENOSYS`. It has since been **rewritten** to the working pattern: mint from a **non-init** user namespace via the `fsopen`/`fsconfig(delegate_*)`/`fsmount` fd-passing split (an init-ns parent sets the delegation options; a child in its own userns owns the bpffs mount and mints the token), hand the token to an unprivileged consumer over `SCM_RIGHTS`, and load with `BPF_F_TOKEN_FD` set (a raw tracepoint, since classic tracepoints attach via `perf_event_open`, which the token does not cover). Run that way it proves end-to-end on stock Ubuntu 6.17 at the hardened default (`unprivileged_bpf_disabled=2`): `=== CH24_PROVEN uid_events=40 token_delegated=yes ===`.
...
That is the hand-off: the attacker never held `CAP_BPF`, only a file descriptor, and the syscall succeeded because the kernel had been told to trust that fd.
...
It is worth being honest about what happened while building this PoC, because the failure modes teach more than the finished shape does.
...
With the architecture and options correct, there is a third gotcha worth recording for anyone who does have `CONFIG_BPF_TOKEN=y`: `BPF_TOKEN_CREATE` can return `EOPNOTSUPP` when the kernel decides the caller is not in `init_user_ns`. The subtlety is that `/proc/self/ns/user` reads from `task->nsproxy->user_ns`, while the kernel's check in `bpf_token_create` reads from `task->cred->user_ns`. In a cloud-init-launched service context those can diverge — `/proc` reports `init_user_ns` while the kernel disagrees — so the mint fails even though the userspace tooling says it should succeed. The way to confirm it on a live box is a kprobe on `bpf_token_create` dumping both fields. None of the kernels available here got that far, because the command was not compiled in, but the caveat matters for deployability: a host can have the feature and still be unable to mint from a given runtime context.

That `init_user_ns` gate is exactly what stopped the shipped PoC — not a missing feature. `BPF_TOKEN_CREATE` does **not** return `ENOSYS` on these kernels: the command is present in the dispatch table on 6.9+, and there is no `CONFIG_BPF_TOKEN` to leave out. The shipped server mints from `init_user_ns` and so hits the `-EOPNOTSUPP` gate above. Reproducing the hand-off means following the non-init-userns `fsopen` choreography from the kernel's own `tools/testing/selftests/bpf/prog_tests/token.c` — and run that way, minting and delegation succeed on the stock Ubuntu 6.17 kernel (verified: `CH24_PROVEN`).

The lesson underneath all of this: **the primitive's reachability is governed by more than kernel version and capability set**. The version check is necessary and the capability check is necessary, but neither is sufficient. The actual `cred->user_ns` has to satisfy the kernel's check too, and that can diverge from what userspace tooling reports. A defender asking "is my host at risk?" has to check not just "does my kernel support bpf_token" (any 6.9+ kernel does — there is no build flag to disable it) but "does `BPF_TOKEN_CREATE` actually succeed from my workload's real runtime context, and is any bpffs mounted with `delegate_*` options."
