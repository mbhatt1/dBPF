...
Each chapter ships with a reproducible trigger that runs under the Docker harness in `dBPF-pocs/`. Every claim has a BEFORE line and an AFTER line printed on stdout, plus a machine-grep-able marker like `CH01_WEAPON_PROVEN flips=2 signals=2` or `CH08_WEAPON_PROVEN exfil_count=13`. The harness records both. 23 PoCs are registered in `dBPF-pocs/harness/proof.py`, covering the 18 on-host technique chapters plus Act 4's three cross-boundary chapters (ch23 TPM unseal, ch24 bpf_token delegation, ch25 IMDS via XDP). Of those 23, 18 fire on the Docker Desktop linuxkit 6.12 aarch64 primary test environment (unchanged from before Act 4 — ch23/ch24/ch25 all skip on linuxkit by design, since linuxkit has no TPM device, no delegated-userns substrate, and no cloud metadata service to intercept). One additional PoC (ch25) fires on the secondary Fedora 42 aarch64 QEMU VM under kernel 6.14 with an XDP mock IMDSv2 on `lo`; the two that pre-Act-4 fired only in Fedora (ch06 LSM, ch12 LSM) still do. Two Act-4 PoCs skip in Fedora too, for environment-specific reasons named honestly in Chapter 21: ch23 has no `/dev/tpm0` or `/dev/tpmrm0` in the test VM, and ch24's `BPF_TOKEN_CREATE` returns `EOPNOTSUPP` on Fedora 42's cloud-init-shaped userns despite `/proc/self/ns/user` matching pid 1. Cross-environment total: 19 fire, 4 skip. Failures and skips are documented honestly inside each chapter and tallied in Chapter 21.
...
```c
// fs/namei.c — do_unlinkat is an internal function; its signature is:
int do_unlinkat(int dfd, struct filename *name)
{
    // ...
    return retval;
}
// Note: do_unlinkat itself does not carry an ALLOW_ERROR_INJECTION annotation.
// The syscall wrapper __arm64_sys_unlinkat (generated by the SYSCALL_DEFINE
// macros) inherits error-injection eligibility automatically.  The entry
// "do_unlinkat" in /sys/kernel/debug/error_injection/list refers to this
// macro-generated wrapper path, not to a hand-written annotation on the
// internal function.
```

The `ERRNO` category shown in the error-injection list declares the injection category: the return value can be overridden to any negative errno. Other categories exist — `TRUE`, `FALSE`, `NULL` — but `ERRNO` is what the filesystem entries use, and it is what `bpf_override_return` targets. A kretprobe can override the return to any value the verifier can produce as an integer, with no further runtime check once the allowlist lookup succeeds.
...
The harness at `dBPF-pocs/harness/proof.py` orchestrates the test matrix. For each chapter, it builds the POC, loads it into a Docker container, runs the trigger, and collects the BEFORE/AFTER output. At the end of the run it writes a single consolidated result file to `/tmp/proof-result.json` — one JSON object per POC with status, event counts, proof-marker capture, verdict, and the present/missing hook lists. There is no per-chapter `results/` directory in the repo; the single run artifact is the definitive record. Reproducing the book's claims means running the harness and diffing against a fresh `/tmp/proof-result.json`.
...
Linuxkit boots in seconds, which matters for the harness. The full test matrix (23 PoCs across the 18 on-host technique chapters plus three Act 4 cross-boundary chapters, each with BEFORE/AFTER capture) runs in under a minute on a clean linuxkit VM. The same matrix on a full Ubuntu VM would take several minutes; on bare metal it would require physical access. The harness's ergonomics were a deciding factor in picking the test environment.
