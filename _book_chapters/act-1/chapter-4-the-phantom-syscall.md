...
The userspace side of this POC lives in `ch04-phantom-syscall.c`. It is 41 lines, which is short enough to read top to bottom without getting lost, but each line does something non-obvious if you have not debugged a libbpf skeleton before. I have, many times, and I still hit the autoattach trap the first time I wrote this one.

Line 27 is the first thing that can fail:
...
Lines 31–33 wire stage 2 into the prog array:
...
`ring_buffer__new` registers the `handle` callback against the ringbuf fd. The third argument is a context pointer passed through to the callback; we pass NULL because the callback needs nothing beyond the event data. The fourth is a `ring_buffer_opts` pointer; NULL uses defaults, which is fine here.

The poll loop blocks for up to 200 ms per iteration waiting for the kernel to signal that new events are available. When the BPF program calls `bpf_ringbuf_submit`, the kernel writes to an epoll fd that `ring_buffer__poll` is watching; the epoll wakes up, `ring_buffer__poll` drains all pending events by calling `handle` once per event, then sleeps again. The 200 ms timeout means the loader checks `stop` at least five times per second for the signal handler to take effect; a timeout of 0 would spin-poll (useful for low-latency testing, bad for a real process).

Lines 39–40 clean up:

```c
ring_buffer__free(rb); ch04_phantom_syscall_bpf__destroy(s);
```

`ring_buffer__free` closes the epoll fd and releases the ringbuf reference. `__destroy` closes the link fd (detaching stage 1 from the tracepoint), closes all map fds, and frees the skeleton struct. The kernel drops its reference to the BPF programs when the last fd referencing them closes; the programs are garbage-collected. After this, `bpftool prog show` sees nothing.

## The ringbuf consumer callback

The `handle` function is nine lines:
