...
The file also declares two additional per-CPU arrays the chapter body doesn't walk step-by-step: `last_ts` (1 entry, per-CPU) caches the previous IRQ timestamp so the handler can compute an inter-IRQ `delta_ns`, and `timing_hist` (8 entries, per-CPU) buckets those deltas into a histogram. The pair turns the POC from a pure observer into a covert timing channel: the loader reads the histogram at shutdown and emits `IRQ_COVERT_CHANNEL_PROVEN timed_events=N` alongside the usual `CH11_PROVEN` marker. The `struct evt` described above also carries an `unsigned long long delta_ns` field not shown in the terse field list.

Four maps total. The first is a standard 1 MiB ringbuf. The sizing matters: on a host with active network traffic and storage IO, you can see multiple IRQs per millisecond. 1 MiB of ringbuf holds roughly 16K events at 64 bytes each; at 1000 events per second, that is a fifteen-second buffer before userspace has to have drained. I picked that bound deliberately — I wanted enough headroom that a slow `printf` in the loader does not start losing events, and small enough that I am not cosplaying as a production-grade pipeline.
...
Three probes on three different rungs of the dispatch ladder. `handle_irq_event` is the mid-layer dispatcher: it runs once per IRQ delivery to a given CPU and kicks off the percpu handler. `__handle_irq_event_percpu` is the inner loop that walks the `irqaction` chain and calls each registered handler. `handle_irq_event_percpu` without the leading underscores is a separate wrapper function in `kernel/irq/handle.c` — it calls `__handle_irq_event_percpu` and then adds entropy via `add_interrupt_randomness` and calls `note_interrupt` for spurious IRQ detection. On some kernel builds — particularly those with aggressive inlining or LTO — this wrapper may be inlined into `handle_irq_event`, causing the symbol to disappear from kallsyms. I added it to the probe set and let the preflight sort it out.
