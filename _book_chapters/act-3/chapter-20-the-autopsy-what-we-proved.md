...
- **REAL** (15 POCs; default category): Hooks the actual kernel subsystem the chapter targets, and can observe or mutate. These are the strongest demonstrations — the hook, the surface, and the effect are all production-representative. Examples: ch01 (cap_capable + `bpf_send_signal`), ch02 (overlayfs copy-up), ch04 (phantom syscall via `tp/syscalls/sys_enter_write`), ch05 (cgroup `cpu.stat` rewrite), ch05b (XDP ghost NIC), ch07 (devcgroup kprobe on `devcgroup_check_permission`), ch08 (keyring heist via kprobe), ch08k (keyring heist kprobe workaround), ch09 (PID-NS cross-mapping + `bpf_send_signal`), ch10 (getdents64 inode cloak), ch11 (IRQ timing channel), ch12 (signed-driver-swap LSM — skips), ch13 (powercap override — skips), ch15 (netns VLAN ghost XDP), ch17 (ACPI/firmware — skips).
- **OBSERVER** (3 POCs): Hooks the real subsystem but cannot mutate — the error-injection allowlist blocks `bpf_override_return` on the target, or atomic context prevents mutation. The POC reads kernel-internal state and exfiltrates it, which is Class III behavior. Members: ch03 (audit record exfil), ch06 (SELinux — skips on non-SELinux kernels), ch16 (seccomp decision leak).
...
| ch09 | real | III | `SEC("raw_tp/sched_process_fork")` + kprobe `alloc_pid` | ringbuf exfiltration + `bpf_send_signal(SIGUSR1)` (armed via `cfg` map) | Cross-namespace PID mapping exposed; SIGUSR1 delivered on match (`CH09_PROVEN host_pid=N mapped=yes` / `PID_NS_ESCAPE_PROVEN` / `SIGUSR1_SENT`) | Category=`real` per `proof.py` (default) |
