...
- **ANALOG** (0 POCs): No analog variants are registered in the current harness. Earlier drafts included analog PoCs for subsystems missing from aarch64 linuxkit (RAPL, ACPI) and for LSM shapes on kernels without SELinux enforcement; these were removed in the cleanup that also produced the current 20-PoC manifest. The category is retained in the dataclass schema for forward compatibility.
