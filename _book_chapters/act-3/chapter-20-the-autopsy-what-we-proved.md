...
The harness runs 20 POCs on the primary test environment (kernel 6.12.54-linuxkit aarch64 via Docker Desktop). 18 produce `_PROVEN` proof markers on stdout there; 2 (ch06 LSM, ch12 LSM) skip on linuxkit at runtime because linuxkit does not load SELinux policy and is not built with `CONFIG_MODULE_SIG_FORCE`, and fire on the Fedora 42 aarch64 QEMU VM secondary (driven by `dBPF-pocs/run-qemu-tests.sh` + `dBPF-pocs/qemu-runner.sh`). Across both environments, all 20 demonstrate. Chapter 21 accounts for the skip-then-fire pair. This chapter walks the 20 proven cases, puts them in five primitive classes, and classifies each into one of four honesty categories. The five-class taxonomy and the four-category system together are the durable artifact of this book.
