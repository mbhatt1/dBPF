...
| **III — Ringbuf exfiltration** | ch03, ch04, ch06o, ch08, ch08k, ch09, ch11, ch16, **ch23 (TPM unseal heist — hardware-rooted key material; fires on any host with a TPM device or swtpm, skips on the TPM-less test environments)** | Cannot prevent at kernel level once the peer has `CAP_BPF`; minimize holders (step 2); ship off-host synchronously (step 5); enclaves for true secrets (step 8); for ch23 specifically, treat any `CAP_BPF` holder on a host with TPM-backed trusted keys as having seen those keys in plaintext — rotate the key set on `CAP_BPF` holder install/update | Audit `bpf(2)` for `BPF_MAP_CREATE` of type `BPF_MAP_TYPE_RINGBUF` and baseline BPF program set; alert on any kprobe attachment to `tpm2_unseal_trusted` or `trusted_key_unseal` outside of known TPM test tooling | Record ringbuf maps created and by whom; correlate with credential access events in other subsystems; for ch23, correlate TPM audit log entries with BPF load events |
...
| **Threat-model subversion via delegated capability** (new in Act 4; does not fit cleanly into Classes I–V because the mechanism is a legitimate kernel feature — `BPF_TOKEN_CREATE` — used for its documented purpose, with the subversion being the trust model around who receives the token fd) | ch24 (bpf_token delegation — skips in both test environments with `CH24_SKIP reason="BPF_TOKEN_CREATE failed: Operation not supported"` on Fedora 42 aarch64 kernel 6.14 under cloud-init; the C code is production-reviewed and the primitive is real on any kernel where the path is live) | Audit every `BPF_TOKEN_CREATE` call and treat the token fd as a transferable capability — track it across SCM_RIGHTS handoffs; restrict which processes may call `BPF_TOKEN_CREATE` via BPF LSM `bpf_token_create` hook if available; do not grant `CAP_BPF` to services that accept unauthenticated client connections | Audit `bpf(2)` for cmd `BPF_TOKEN_CREATE` (cmd 36); audit SCM_RIGHTS on unix sockets owned by `CAP_BPF`-holding processes; correlate token creation with unexpected client uids using the resulting fd | Kernel audit log shows token create by a server process followed by token use by a client with a different uid; orchestrator's capability inventory (step 2) shows a delegated-capability footprint that was not present at baseline |
