...
| **III — Ringbuf exfiltration** | ch03, ch04, ch06o, ch08, ch08k, ch09, ch11, ch16 | Cannot prevent at kernel level once the peer has `CAP_BPF`; minimize holders (step 2); ship off-host synchronously (step 5); enclaves for true secrets (step 8) | Audit `bpf(2)` for `BPF_MAP_CREATE` of type `BPF_MAP_TYPE_RINGBUF` and baseline BPF program set | Record ringbuf maps created and by whom; correlate with credential access events in other subsystems |
