...
On a stock x86 kernel with `CONFIG_ACPI=y`, the ACPI namespace is populated at boot from DSDT and SSDT. Various subsystems — thermal, battery, lid switch, WMI (Windows Management Instrumentation) — make calls through `acpi_evaluate_object(handle, pathname, external_params, return_buffer)`. The `pathname` parameter is the second argument, typed `acpi_string` (a `char *` typedef), pointing to caller-allocated memory filled in with an AML pathname like `"\_SB._WS0.Ping"`.
