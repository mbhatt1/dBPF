...
Twenty-one chapters (eighteen on-host plus three Act 4 cross-boundary), two kernels (Docker Desktop linuxkit 6.12 aarch64 primary; Fedora 42 aarch64 QEMU 6.14 secondary), one harness. This chapter draws a line under what was actually shown and what was not.

## What was demonstrated

Twenty-three POCs are registered in `dBPF-pocs/harness/proof.py` and run under the reproducible Docker harness on kernel 6.12.54-linuxkit aarch64. The final run on that primary environment splits as follows: **18 produced the primitive effect** and printed their `_PROVEN` proof marker; **2 skip on linuxkit at runtime** (ch06 LSM, ch12 LSM) and fire instead on the secondary Fedora 42 aarch64 QEMU VM that has SELinux enforcing and module signature enforcement; **3 Act 4 PoCs skip on linuxkit by design** (ch23 no TPM device, ch24 no delegated-userns substrate, ch25 no metadata service) and are exercised against the Fedora QEMU VM — one of them (ch25) fires there end-to-end via an XDP mock IMDSv2 on `lo`, two skip honestly for named environmental reasons (ch23 no `/dev/tpm0`, ch24 `BPF_TOKEN_CREATE` returning `EOPNOTSUPP` under cloud-init); **0 failures**. The POC list in `proof.py` is the authoritative manifest — 18 "primary" on-host chapters (ch01 through ch18, plus ch05b), two kept variants (ch06o kprobe-observer of SELinux, ch08k kprobe variant of keyring heist), one illusion variant (ch12s, the syscall kretprobe forge of `finit_module`), and three Act 4 cross-boundary chapters (ch23, ch24, ch25). All 23 are in one table.
...
Five POCs skip on the linuxkit primary environment at runtime. All are registered in `proof.py`; none is a failure.
...
**ch23 (TPM unseal heist)** — Act 4 addition. A kprobe on `tpm2_unseal_trusted` that copies plaintext key bytes out of the kernel's TPM unseal path to ringbuf, proving that `CAP_BPF` reaches hardware-rooted trusted keys after the TPM has performed the unseal. Linuxkit has no TPM device; the Fedora 42 QEMU VM as provisioned also has no TPM (`tpm: (empty)` in the run log), so the trigger prints `CH23_SKIP reason="no /dev/tpm0 or /dev/tpmrm0"` and the PoC defers to an environment with swtpm passthrough or real TPM hardware. The PoC's BPF code is production-reviewed; the skip is environmental, not a weakness of the primitive.

**ch24 (bpf_token delegation)** — Act 4 addition. Demonstrates threat-model subversion via delegated capability: a privileged server holding `CAP_BPF` creates a `BPF_TOKEN_CREATE` fd, hands it to an unprivileged client via SCM_RIGHTS, and the client uses the token to attach a tracepoint as if it held the capability itself. On Fedora 42 aarch64 kernel 6.14 under cloud-init, `BPF_TOKEN_CREATE` returns `EOPNOTSUPP` — even though `/proc/self/ns/user` matches pid 1 and the C code is sound after seven iterations of increasingly specific failure-mode chasing. This is a Fedora-on-cloud-init observation about the kernel's bpf-token delegation path in that environment, not a mainline kernel bug. The PoC is registered so that anyone reproducing on a kernel where the path is live sees the primitive; the skip is documented as a scar in Chapter 24.

**ch25 (metadata faucet via XDP)** — Act 4 addition. An XDP program attached to the loopback interface intercepts IMDSv2 requests to `169.254.169.254` and harvests access keys and session tokens from responses before the workload sees them. On linuxkit there is no metadata service to intercept. On the Fedora 42 QEMU VM the PoC runs in "mock" mode against a local mock IMDSv2 on `lo`, the XDP program attaches cleanly, and the marker fires: `CH25_PROVEN access_key_captured=yes token_captured=yes role=demo-role`. This is the Act 4 chapter that fires on the QEMU secondary end-to-end.
...
- **1 additional fire on the Fedora QEMU VM** that does not fire on linuxkit: ch25 (XDP mock IMDS on `lo`, `CH25_PROVEN access_key_captured=yes token_captured=yes role=demo-role`).
- **2 skip in both environments with named Act-4 reasons**: ch23 (no TPM device in either test environment) and ch24 (`BPF_TOKEN_CREATE` returns `EOPNOTSUPP` on Fedora 42 under cloud-init).
- **0 failures.**

Counting the stricter bar — only `real`-category POCs that actually demonstrated effect on linuxkit — gives 12. Counting all `real`-category POCs that fire in either environment (including ch12 and ch25) gives 14. The across-environment total is 19 demonstrated, 4 skipped with honest named reasons.
