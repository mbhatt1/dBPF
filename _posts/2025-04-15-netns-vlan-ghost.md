...
> **See also**: [Book chapter with investigation notes]({{ site.baseurl }}/book/act-3/chapter-15-netns-vlan-ghost.html) · [POC code](https://github.com/mbhatt1/dBPF/tree/master/dBPF-pocs/pocs/ch15-netns-vlan-ghost)
...
The `tcpdump` frame is untagged — no `vlan 4242` in the dissector output, inner ethertype `0x88b5` (IEEE Local Experimental) is the outermost now. That is evidence the strip ran before the egress veth saw the packet.
...
- Docker Desktop linuxkit aarch64: veth drivers do not implement native XDP; the loader falls back to SKB-mode (or `--skb` skips the probe). Native-mode redirect is unavailable.
...
---
**Related material**
- Full chapter: [Chapter 15 — NetNS VLAN Ghost]({{ site.baseurl }}/book/act-3/chapter-15-netns-vlan-ghost.html)
- POC source: [dBPF-pocs/pocs/ch15-netns-vlan-ghost/](https://github.com/mbhatt1/dBPF/tree/master/dBPF-pocs/pocs/ch15-netns-vlan-ghost)
- Harness entry: `Poc("ch15", ...)` in `proof.py`
