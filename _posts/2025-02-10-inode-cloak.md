...
# Inode Cloak: Making Files Vanish from getdents64

> **See also**: [Book chapter with investigation notes]({{ site.baseurl }}/book/act-2/chapter-10-inode-cloak.html) · [POC code on GitHub](https://github.com/mbhatt1/dBPF/tree/master/dBPF-pocs/pocs/ch10-inode-cloak)
...
`bpftool prog show` lists both tracepoint programs. The loudest tell is `bpftool map dump name hidden`, which prints the cloaked filename set in cleartext — the attacker has to keep the map populated for the cloak to fire. `strace -e getdents64` sees the post-mutation buffer, but a syscall-level comparator (a second, higher-priority tracepoint; a userspace reader that mmaps `/proc/self/mem` of a tracer; or a differently-routed readdir path) can diff against the unmodified stream. Cross-referencing `ls` output with `find -inum` exposes the discrepancy immediately.
