...
# IRQ Affinity Chaos: What the IRQ Dispatch Path Will and Won't Let You Do

> **See also**: [Book chapter with investigation notes]({{ site.baseurl }}/book/act-2/chapter-11-irq-affinity-chaos.html) · [POC code on GitHub](https://github.com/mbhatt1/dBPF/tree/master/dBPF-pocs/pocs/ch11-irq-chaos)

I started this one wanting to rewrite CPU affinity masks inside `irq_dispatch()` from BPF. That's the fantasy. What the POC actually ships is an IRQ observer, because the override path is refused at the verifier by the kernel's error-injection allowlist, and the real re-steering lives below the Linux IRQ layer on aarch64 anyway. The previous version of this post described the override as if it worked. It didn't. Below is what the POC does and why.
...
Three kprobe targets sit on overlapping rungs of the dispatch ladder. The loader preflights each against `/proc/kallsyms` and disables `autoload` on any that don't resolve, so the `load` stays clean on kernels that only have a subset:
...
Two out of three fire on this kernel. The third is a naming variant that never resolved; the preflight keeps the load error-free. CO-RE walks pull the IRQ number out of `irq_desc.irq_data.irq` and the driver label out of the first `irqaction.name`, bump a per-CPU BPF hash counter, and push a ringbuf event.
...
`emit()` reads `desc->irq_data.irq` and `desc->action->name` with `BPF_CORE_READ`, bumps the per-CPU HASH counter keyed by irq number, and submits a ringbuf event containing `{ts_ns, cpu, irq, pid, comm, name, hook}`.

## Verification

`trigger.sh` runs a BEFORE/AFTER against a `ping -c 20 127.0.0.1` plus `dd if=/dev/urandom of=/dev/null bs=1M count=20` workload to generate net-RX and block/crypto interrupts. `/proc/interrupts` provides the BEFORE baseline — coarse counters, no per-event timing. The BPF observer provides the AFTER delta.
...
Factual note: the original post claimed hooks on `irq_dispatch()` with inline affinity-mask rewrites. That wasn't the POC's behaviour and, per the three blockers above, isn't achievable on this kernel from BPF. The observer is honest; the override was wishful thinking.
