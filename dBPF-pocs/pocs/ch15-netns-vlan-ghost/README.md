...
**Note:** Docker must be run with `--net=host` so the XDP program can attach
to host network interfaces (veth pairs created in the host netns). Without
`--net=host` the container's network namespace is isolated and the veths are
not visible.

## Mechanism
An XDP program attached to the host-side of a veth pair inspects every
ingress frame. When it finds an 802.1Q header with VID 142
(`COVERT_VLAN_ID`) it:
...
The result: traffic tagged with VID 142 on `veth_host` can be ferried
...
VLAN-142-tagged AF_PACKET frame from inside ghost_a, sniffs egress on
veth_host2):
```
...
container with `--net=host` (Ubuntu 6.17 aarch64 Lima VM, XDP forced to SKB-mode):
```
...
=== sending 3 VLAN-142 AF_PACKET frames from inside ghost_a ===
sent 3 VLAN-142 covert frames

=== loader ringbuf events ===
[vghost] ifin=42 vid=142 inner=0x88b5 ee:f0:6e:11:22:33 -> ff:ff:ff:ff:ff:ff  STRIP+REDIRECT ifout=44
[vghost] ifin=42 vid=142 inner=0x88b5 ee:f0:6e:11:22:33 -> ff:ff:ff:ff:ff:ff  STRIP+REDIRECT ifout=44
[vghost] ifin=42 vid=142 inner=0x88b5 ee:f0:6e:11:22:33 -> ff:ff:ff:ff:ff:ff  STRIP+REDIRECT ifout=44
...
```
The tcpdump frame is **untagged** (no `vlan 142` in the dissector
...
- **Ubuntu 6.17 aarch64 (Lima VM)**: veth drivers do NOT implement
  native XDP; the loader falls back to SKB-mode (or use `--skb` to
  skip the probe). Native-mode redirect is unavailable.
- **`--net=host` required**: the Docker run command must include
  `--net=host` so the container can see and attach XDP to the host-side
  veth interfaces. Without it the veths are hidden in the container's
  own netns and the loader cannot find them.
...
- `arping` and `ping` won't generate the covert frames reliably; the
  trigger uses an embedded AF_PACKET sender for deterministic VLAN-142
