...
`/sys/kernel/debug/error_injection/list` on Docker Desktop's linuxkit
6.12 aarch64 kernel, so `bpf_override_return(ctx, 0)` from a kretprobe
takes effect: glibc receives `0`, programs like `id`/`whoami` then ask
NSS for "the user with uid 0" and print `root`.
...
Captured runtime output (linuxkit 6.12 aarch64, --all mode):
...
See the chapter write-up: [`2025-05-09-ebpf-token-bypass`](../../../_posts/2025-05-09-ebpf-token-bypass.md) in the Diabolical eBPF Field Manual.
