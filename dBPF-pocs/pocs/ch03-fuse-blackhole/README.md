...
The kernel audit subsystem calls `audit_log_start` only when a consumer is
registered on the netlink socket. Without `auditd`, the kernel skips
`audit_log_start` entirely and the kprobes see no events. Start auditd first:
