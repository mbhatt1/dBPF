...
The kretprobe in the sibling `ch01-mirror-controls` also delivers
`bpf_send_signal(SIGUSR1)` on denials (field `signal_sent`, tag `FLIP`).
This LSM variant performs the real fmod_ret override: the `-a` wildcard
flag is supported in the loader.
