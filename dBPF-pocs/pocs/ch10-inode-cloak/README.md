...
**Source fix applied**: The BPF map originally named `active` collided with
the kernel enum value `active = 6` in `vmlinux.h`, causing a compile error.
The map has been renamed from `active` to `active_calls` in
`ch10-inode-cloak.bpf.c`. This fix is now in the source; a clean `make`
will succeed.
