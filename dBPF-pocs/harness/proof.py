...
POCS: list[Poc] = [
    ...
    Poc("ch02lsm", "OverlayFS Trojan (BPF LSM copy-up deny)", "ch02-overlayfs-lsm",
        ...
        loader_args=["-p", "secret.txt"],  # basename the trigger copies up; without it the loader protects nothing -> 0 events
    ...
    Poc("ch06", "SELinux LSM observer (flip is impossible: deny-wins, bpf ordered after selinux)", "ch06-silence-selinux-lsm",
        hooks=["bpf-lsm"], prefix="[ch06]",
        proof_marker=r"CH06_OBSERVER_PROVEN",
        category="observer"),
